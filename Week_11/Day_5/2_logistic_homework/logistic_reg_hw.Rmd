...
```{r}
regsubsets_backward <- regsubsets(purchase  ~ ., 
                                 data = orange_juice, 
                                 method = "backward")
```
...
```{r}
summary(regsubsets_backward)$which[8,]
```
```{r}
orange_juice
```
...
```{r}
mod_8 <- glm(purchase ~ weekof_purchase + store_id +
             price_ch + price_mm + disc_ch +
             disc_mm + special_ch + special_mm,
             data = orange_juice,
