...
```{r}
Beverage <- drinks_content %>% 
            select(1:4) %>% 
            filter(Calories != TRUE)
Beverage
```
...
```{r}
# 135 Claories = 10 cups of suger
Beverage <- Beverage %>% 
            mutate(calorie_diff = Calories - 135)  
  
Beverage
# ran out of time to work this out
#if(Beverage, calorie_diff < 0, calorie_diff = "Less")
```
...
```{r}
calories_by_category <- Beverage %>%
  
      group_by(Beverage_category) %>% 
      summarise(mean = mean(Calories)) %>% 
      arrange(desc(mean(Calories)))
...
```{r}
# av_calories <- Beverage %>% 
#       group_by(Beverage_category, Calories) %>% 
#       mutate(Calories = sum(Calories)/nrow())
# av_calories
```
```{r}
...
```{r}
# average_bev_prep %>% 
#     filter(mean_cal < mean_calories)
```
```{r}
average_bev_prep <- Beverage %>%
