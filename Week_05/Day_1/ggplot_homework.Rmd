...
```{r}
  ...
  ggplot(data = qb_revenue_breakdown, mapping = aes(x = Product, y = Revenue, fill = Year)) +
...
ggplot(kwikbit_sales, aes(x = Date, y = Amount) ) +
  geom_line() +
...
# Was only taking the first argument 
...
<br>
```{r}
ggplot(qb_competitors, aes(Year, Revenue, fill = Company)) +
  geom_area() +
  theme(axis.text = element_text(size = 12),
        plot.title = element_text(size = 18)
        ) +
  labs(
    title = "Annual Sales",
    subtitle = "fitness trackers over the last 5 years\n"
  )
```
...
<br>
```{r}
ggplot(qb_competitors, aes(Revenue, Year, colour = Company)) +
  geom_line() +
  theme(axis.text = element_text(size = 12),
        plot.title = element_text(size = 18)
        ) +
  labs(
    title = "Annual Sales Comparason",
    subtitle = "fitness trackers over the last 5 years\n"
  )

ggplot(qb_competitors, aes(Year, Revenue, colour = Company)) +
  geom_line() +
  geom_point() +
  theme(axis.text = element_text(size = 12),
        plot.title = element_text(size = 18)
...
<br>
```{r}
no_idea <- qb_device_data %>% 
            group_by(device, hours) %>% 
            summarise(counts)

ggplot(no_idea, aes(counts, hours, colour = device)) +
  geom_line() +
  labs(
    title = "Step Counts",
    subtitle = "Step Counts v Competitors\n"   # \n makes a new row
  ) +
  theme(axis.text = element_text(size = 12),
        plot.title = element_text(size = 18),
        panel.background = element_rect(fill = "white", colour = "grey50")) +
  scale_x_continuous(trans = "log10", name = "log(counts)") +
  scale_y_continuous(trans = "log10", name = "log(hours)") +
